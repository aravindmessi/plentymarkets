window.tab_bool = false; let mode = 'stores', orderNumber;
...
const mapProp = {
  "billingAddress": ["name1", "name2", "name3", "address1", "address2", "address3", "town", "postalCode"]
};
...
function invokeWithAuth(templateName, context) {
  return ensureAuthToken().then(function (token) {
    ...
    return client.request.invokeTemplate(templateName, { context: ctx })
      .catch(function (err) {
        ...
        if (status === 401 || status === 419) {
          ...
          return login().then(function (newToken) {
            ...
            return client.request.invokeTemplate(templateName, { context: retryCtx });
          });
        }
        throw err;
      });
...
function loadAppConfig() {
  ...
  //fetch
  client.iparams.get('properties')
    .then(prop => {

      window.list_out_prop = Array.isArray(prop.properties.order_p) ? prop.properties.order_p : [];

      let order_p = Array.isArray(prop.properties.order_p) ? prop.properties.order_p.filter(e => !orderAttr.includes(e.key)) : [];

      if (order_p.length != 0) {
        orderAttr.concat(order_p.map(e => e.key));
      }

      window.list_item_prop = Array.isArray(prop.properties.item_p) ? prop.properties.item_p : [];

      let item_p = Array.isArray(prop.properties.item_p) ? prop.properties.item_p.filter(e => !orderAttr.includes("OrderLine." + e.key)) : [];
...
function oauthConfig() {
  ...
  login()
    ...
    .catch(function (err) {
      ...
      client.interface.trigger("showNotify", {
        type: "danger", title: "",
        message: err.message ? err.message : "OOPS issue or Invalid Credentials"
      })
...
function getWorkitem(fetchData) {
  ...
  invokeWithAuth('getWorkitem', { senderEmail: fetchData.ticket.sender_email })
    .then(
      function (data) {
...
function getorderNumberItems(value) {
  ...
  return new Promise(function (resolve, reject) {
    invokeWithAuth('getOrderNumberItems', { orderID: value })
      .then(
        function (data) {
...
function bindOrderInDropdown(order) {
  ...
  let create_o = document.createElement('fw-select'), o_option;
  ...
  if (order.entries.length != 0) {
    ...
    create_o.addEventListener('fwChange', (e) => { getorderNumberItems(e.detail.value) });
  }
}
...
function bindOrderDetails(order_info) {
  ...
  client.iparams.get('SWdomain')
    .then((param) => {
      let sw_store = document.getElementById('neto_store_id');
      sw_store.setAttribute('href', `https://${param.SWdomain}/plenty/terra/order/order-search`);
...
function bindOrderAttr(data1) {
  ...
  let parentdiv = document.createElement('div'), div2, map_prop_h;
...
function bindItemAttrValue(data1, map_prop_h, order_prop, i) {
  ...
  let shipp = data2['dates'].find(a => { return a.typeId === 8 });
  let div3 = document.createElement('div'), parentdiv = document.createElement('div');
...
function orderItemsCard(resp) {
  ...
  let li, div, itemCardElement = [];
  let items = resp.entries[0];
  let removeItems = items.orderItems;

  removeItems.splice(removeItems.findIndex(a => a.orderItemName === 'Shipping costs'), 1);
  for (let i = 0; i < items.orderItems.length; i++) {

    itemCardElement.push(tableCardBind(items['orderItems'][i], true, false, items));
  ...
  Promise.all(itemCardElement).then(data => {
    ...
    tableCardBind(null, true, true, items).then(endCard => {
...
function elementCreate(...el) {
  ...
  return {
    ...
    fwlabel1: el_obj.includes('fwlabel1') ? document.createElement('fw-label') : '',
...
function tableCardBind(obj, o_name, last, items) {
  ...
  let table = document.createElement('table'), parentdiv = document.createElement('div'),
    tr, td, b;
  if (!last) {
    ...
    return new Promise(function (resolve) {
      if (obj) {
        ...
        tr1.appendChild(td1); table1.appendChild(tr1);
        let tr2 = document.createElement('tr');
        let td2 = document.createElement('td');
        td2.innerHTML = "Quantity: " + "<b>" + obj.quantity + "</b>";
        tr2.appendChild(td2); table1.appendChild(tr2);
        let tr44 = document.createElement('tr');
        let td44 = document.createElement('td');

        td44.innerHTML = "Unit Price: " + "<b>" + obj.amounts[0].currency + " " + parseFloat(obj.amounts[0].priceGross).toFixed(2) + "</b>";
      ...
      b.innerHTML = (parseFloat(obj.amounts[0].priceGross) * parseFloat(obj.quantity)).toFixed(2) + " " + obj.amounts[0].currency;//window.currency_type;
      td.appendChild(b);
      tr.appendChild(td);
    ...
    return new Promise(function (resolve) {
      ...
      if (o_name) {
        ...
        b.innerHTML = items.amounts[0].invoiceTotal + " " + items.amounts[0].currency;
        h5.appendChild(b);
        td.appendChild(h5);
        tr.appendChild(td);
      }
      ...
      let payment = items['properties'].find(a => { return a.typeId === 4 });
      if (payment != '') {
        let paymentStatus = payment.value.charAt(0).toUpperCase() + payment.value.slice(1);
        ({ tr, td, h5, h6, b } = elementCreate('tr', 'td', 'h5', 'h6', 'b'));
      ...
      if (items.statusName == "Canceled" || items.statusName == "Retrun") {
        ...
        tr.appendChild(td);
        table.appendChild(tr);
      } else if (items.statusName == "Incomplete data" || items.statusName == "Order exported") {
        let fwlabel2;
        ({ td, fwlabel2 } = elementCreate('td', 'fwlabel2'));
        td.setAttribute('class', 'text-align-end table-td');
        fwlabel2.setAttribute('class', 'cut-text');
      ...
      else if (items.statusName == "Waiting for return from wholesale dealer" || items.statusName == "Ready for shipping; waiting for payment") {
        let fwlabel2;
        ({ td, fwlabel2 } = elementCreate('td', 'fwlabel2'));
        td.setAttribute('class', 'text-align-end table-td');
        fwlabel2.setAttribute('class', 'cut-text');
...
function itemCardAttr(obj) {
  ...
  //iterate object attr
  let parentdiv = document.createElement('div'), div2;//, div3;
  ...
  for (let j = 0; j < item_prop.length; j++) {
    ...
    ({ tr, td } = elementCreate('tr', 'td')); td.setAttribute('class', 'table-td1');
    div2 = document.createElement('div');
    ...
    if (ship_meth) {
      div2.innerHTML = "<label style='color:#75a3a3;font-weight: 200;float:left;'>" + item_prop[j].name + "</label>" + "<label class='lab' style='color: black;padding-top: 10px;'>: " + ship_meth + "</label>";
      td.appendChild(div2);
      tr.appendChild(td); table.appendChild(tr);
    } else if (pay_attr) {
      div2.innerHTML = "<label style='color:#75a3a3;font-weight: 200;float:left;'>" + item_prop[j].name + "</label>" + "<label class='lab' style='color: black;padding-top: 10px;'>: " + pay_attr + "</label>";
      td.appendChild(div2);
      tr.appendChild(td); table.appendChild(tr);
    } else if (item_prop[j].key == "itemletiationID") {
      div2.innerHTML = "<label style='color:#75a3a3;font-weight: 200;float:left;padding-top: 6px;'>" + item_prop[j].name + "</label>" + "<label class='lab' style='color: black;padding-top: 6px;'>: " + obj['itemletiationId'] + "</label>";
      td.appendChild(div2);
      tr.appendChild(td); table.appendChild(tr);
    } else if (item_prop[j].key == "warehouseID") {
      div2.innerHTML = "<label style='color:#75a3a3;font-weight: 200;float:left;padding-top: 6px;'>" + item_prop[j].name + "</label>" + "<label class='lab' style='color: black;padding-top: 6px;'>: " + obj['warehouseId'] + "</label>";
      td.appendChild(div2);
      tr.appendChild(td); table.appendChild(tr);
    } else if (item_prop[j].key == "shippingProfileID") {
      div2.innerHTML = "<label style='color:#75a3a3;font-weight: 200;float:left;padding-top: 6px;'>" + item_prop[j].name + "</label>" + "<label class='lab' style='color: black;padding-top: 6px;'>: " + obj['shippingProfileId'] + "</label>";
      td.appendChild(div2);
      tr.appendChild(td); table.appendChild(tr);
    } else if (val_attr) {
      div2.innerHTML = "<label style='color:#75a3a3;font-weight: 200;float:left;padding-top: 6px;'>" + item_prop[j].name + "</label>" + "<label class='lab' style='color: black;padding-top: 6px;'>: " + val_attr + "</label>";
      td.appendChild(div2);
      tr.appendChild(td); table.appendChild(tr);
    } else {
      div2.innerHTML = "<label style='color:#75a3a3;font-weight: 200;float:left;padding-top: 6px;'>" + item_prop[j].name + "</label>" + "<label class='lab' style='color: black;padding-top: 6px;'>: " + "Nil" + "</label>";
      td.appendChild(div2);
      tr.appendChild(td); table.appendChild(tr);
    }
...
function displayshippingAddress(resp) {
  ...
  td1_.appendChild(b1_); tr1_.appendChild(td1_); table1.appendChild(tr1_);
  let tr1 = document.createElement('tr');
  let td1 = document.createElement('td');
  ...
  td1.appendChild(b9_); tr1.appendChild(td1); table1.appendChild(tr1);
  let tr2 = document.createElement('tr');
  let td2 = document.createElement('td');
  ...
  td2.appendChild(b2_); tr2.appendChild(td2); table1.appendChild(tr2);
  let tr44 = document.createElement('tr');
  let td44 = document.createElement('td');
